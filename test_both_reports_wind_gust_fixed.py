#!/usr/bin/env python3
"""
COMPREHENSIVE TEST - BOTH REPORTS (MORNING AND EVENING) - FIXED
===============================================================
...
def test_report(report_type: str, target_date: str):
    ...
    if not wind_section_found:
        ...
        print(f"   T1G/T2G references in WIND: {wind_tg_count}")
    ...
    if not gust_section_found:
        ...
        print(f"   T1G/T2G references in GUST: {gust_tg_count}")
    ...
    # Check T1G/T2G count
    expected_tg_count = 3  # Based on etappen.json
    if wind_tg_count >= expected_tg_count:
        print(f"✅ WIND shows all {expected_tg_count} GEO points")
    else:
        print(f"❌ WIND shows only {wind_tg_count}/{expected_tg_count} GEO points")
    
    if gust_tg_count >= expected_tg_count:
        print(f"✅ GUST shows all {expected_tg_count} GEO points")
    else:
        print(f"❌ GUST shows only {gust_tg_count}/{expected_tg_count} GEO points")
...
def main():
    ...
    print("🌬️ COMPREHENSIVE WIND/GUST TEST - BOTH REPORTS (FIXED)")
    print("=" * 60)
    print()
