#!/usr/bin/env python3
"""
Real Live Test with actual email sending.
Tests the compact formatter with real weather data and sends an actual email.
"""
...
def test_real_email_live():
    """Test the compact formatter with real weather data and send actual email."""
    
    print("🌤️  REAL LIVE TEST - Email Sending with MeteoFrance API")
    print("=" * 60)
    ...
    try:
        ...
        print(f"   SMTP host: {config_dict.get('smtp', {}).get('host', 'unknown')}")
        print(f"   Email to: {config_dict.get('smtp', {}).get('to', 'unknown')}")
        ...
        email_content = f"""
🌤️ GR20 Weather Report - {stage_name}
...
• Stage: {stage_name}
• Date: {date.today()}
• Data Source: {real_weather_data.data_source}
• Max Temperature: {real_weather_data.max_temperature}°C
• Min Temperature: {real_weather_data.min_temperature}°C
• Max Rain Probability: {real_weather_data.max_rain_probability}%
• Max Precipitation: {real_weather_data.max_precipitation}mm
• Max Wind Speed: {real_weather_data.max_wind_speed} km/h
• Max Wind Gusts: {real_weather_data.max_wind_gusts} km/h
• Max Thunderstorm Probability: {real_weather_data.max_thunderstorm_probability}%

🎯 This is a REAL LIVE TEST with:
• Real weather data from MeteoFrance API
• Compact formatter integration
...
"""
        ...
        print("-" * 40)
        print(f"⚠️  This will send an actual email to: {config_dict.get('smtp', {}).get('to', 'unknown')}")
        print(f"📧 Subject: {email_subject}")
        print(f"📧 Content length: {len(email_content)} chars")
        print()
        
        # Initialize email client
        email_client = EmailClient(config_dict)
        ...
        if email_success:
            ...
            print(f"   Email delivered to: {config_dict.get('smtp', {}).get('to')}")
            print(f"   Subject: {email_subject}")
        ...
        if config.use_compact_formatter:
            ...
            print(f"🎯 This is a TRUE LIVE TEST with actual API calls and EMAIL SENDING!")
        else:
            print(f"🎯 Using standard format with REAL weather data!")
            
        print(f"\n🌤️  REAL LIVE TEST COMPLETED!")
        print(f"   Real weather data from MeteoFrance API")
        print(f"   Compact formatter working with live data")
        print(f"   Real email sent to: {config_dict.get('smtp', {}).get('to')}")
