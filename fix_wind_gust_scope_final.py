#!/usr/bin/env python3
"""
FIX WIND AND GUST SCOPE FINAL
=============================
Fix the scope issue by placing WIND and GUST debug sections in the correct location.
"""
...
# Find where the WIND and GUST sections should be placed (after RAIN sections)
# Look for the end of the RAIN sections
rain_end_marker = '''                debug_lines.append("")'''

# Find the location where we should insert WIND and GUST
# It should be after the RAIN sections but before THUNDERSTORM
insert_location = '''                debug_lines.append("")
                
                # Wind data debug (analog to Rain) - show ALL n GEO points
...
# Replace the rain end marker with the new content
content = content.replace(rain_end_marker, insert_location)

# Remove the old WIND and GUST sections that are outside the method
# Find and remove the old sections
...
# Remove the old WIND section
content = content.replace(old_wind_section, "")

# Also remove the old GUST section
old_gust_section = '''            # Gust data debug (analog to Rain) - show ALL n GEO points
            debug_lines.append("GUST")
...
# Remove the old GUST section
content = content.replace(old_gust_section, "")

# Write back to file
with open('src/weather/core/morning_evening_refactor.py', 'w') as f:
    f.write(content)

print("✅ WIND and GUST scope fixed!")
print("   - Moved WIND and GUST sections to correct location")
print("   - Removed old sections outside the method")
print("   - All variables now properly defined in scope") 