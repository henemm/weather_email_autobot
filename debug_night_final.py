#!/usr/bin/env python3
"""
Debug Night Final - Check T1G3 Marseille temperature
"""
...
def main():
    print("🔍 DEBUG NIGHT FINAL - T1G3 MARSEILLE")
    print("=" * 50)
    
    # T1G3 coordinates (Marseille) - LAST point of today's stage
    lat = 43.283255
    lon = 5.370061
    point_name = "Marseille_T1G3"
    
    print(f"📍 Location: Marseille (T1G3) - LAST point of today's stage")
    ...
    try:
        ...
        # Fetch weather data
        print("🔍 Fetching MeteoFrance data for T1G3...")
        ...
        if 'daily' in daily_forecast:
            ...
            if found_entry:
                ...
                print(f"\n📊 T1G3 MARSEILLE TEMPERATURES (2025-08-02):")
                print(f"  T.min: {temp_min}°C")
                print(f"  T.max: {temp_max}°C")
                
                # Check if this matches what we expect
                if temp_min == 20 or temp_min == 20.0:
                    print(f"  🎯 FOUND 20°C!")
                ...
                print(f"\n🎯 EXPECTED RESULT:")
                print(f"  Night function should show: N{round(temp_min)}")
                print(f"  Debug should show: T1G3 | {temp_min}")
            else:
